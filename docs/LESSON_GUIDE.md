...
이 강좌는 **입문자와 초보 이상(중급) 학습자가 함께** 쓰는 것을 목표로 합니다.
강의자료(PPT)의 내용은 **바닥(최소 기준)** 이고, 거기서 멈추지 말고 실무에서 쓰는 수준까지 끌어올립니다.

### 교시(섹션) 한 개의 구성

1. **도입** — 이 교시에서 풀 문제 · 왜 필요한지 (1~2문단, 가능하면 그림)
2. **기본 개념** — 강의자료 내용 (입문자 기준, 그림 · 표 · 짧은 예제)
3. **예제** — 강의자료 코드 + 변형 예제. 각 예제에 `desc` 로 "무엇을 보는 예제인지" 한 줄
4. **한 걸음 더 (중급)** — `callout kind:'more'` 또는 `h` + 예제로 다음을 다룹니다.
   - 파이썬다운 표현 (f-string, 컴프리헨션, `enumerate`/`zip`, 언패킹, `with`, `pathlib` …)
   - 동작 원리 (메모리 · 참조 · 가변/불변 · 평가 순서 · 예외 흐름 · 이름 공간)
   - 흔한 실수와 오류 메시지 읽는 법, 디버깅 방법(`print`, `type`, `repr`, 작은 예제로 쪼개기)
   - 성능 · 가독성 비교(“이렇게도 되지만 보통은 이렇게 씁니다”), 표준 라이브러리 소개
   - PEP 8 스타일, 타입 힌트, 간단한 테스트(`assert`) — 챕터에 맞을 때
5. **실습 과제** — 아래 기준
6. **확인 퀴즈** — 4~6문항 (개념 2 + 코드 결과 예측 2 + 오류 찾기 1 이상)

### 실습 · 프로젝트 분량 (교시마다)

| 난이도 | 개수 | 성격 |
|---|---|---|
| `level: 1` 기초 | 2~3개 | 방금 배운 문법을 그대로 써 보는 짧은 문제 (5~15줄) |
| `level: 2` 응용 | 2~3개 | 두세 개념을 엮는 문제, 입력 · 조건 · 반복 조합 (15~40줄) |
| `level: 3` 도전 | 1~2개 | 스스로 설계해야 하는 문제. 실전에 가까운 작은 프로그램 |

- 교시마다 **실습 5개 이상**, 챕터마다 **미니 프로젝트 1개 이상**(`level: 3`, 제목 앞에 `🚀 프로젝트`)을 넣습니다.
- 프로젝트는 요구 사항을 **목록으로 분명히** 적고(입력 · 출력 · 규칙 · 예시 실행 장면), `starter` 에 뼈대와 TODO,
  `solution` 에 완성 코드를 넣습니다. 가능하면 **단계별 확장 아이디어**("여기까지 했다면 이렇게 더 해 보세요")를 덧붙입니다.
- 강의자료의 SELF STUDY · 연습문제는 그대로 포함하고(번호 유지), 그 위에 새 문제를 더합니다.
- 실습마다 `hint`, 가능하면 `expect`(정답 코드의 실제 출력)를 넣습니다.

### 중급 학습자를 위한 배려

- 이미 아는 사람이 건너뛸 수 있게 소제목(`h`)으로 흐름을 나눕니다.
- `callout kind:'more'` 제목에 다루는 주제를 적습니다. 예: `title: 'f-string 과 format() 비교'`
- “왜 이렇게 쓰는가”를 설명합니다. 문법 나열이 아니라 **선택의 이유**(읽기 쉬움 · 안전 · 성능)를 함께 적습니다.
- 표준 라이브러리 · 생태계로 이어지는 길을 알려 줍니다 (`random`, `datetime`, `json`, `csv`, `pathlib`, `collections`,
  `itertools`, `sqlite3`, `pip`, 가상환경 등 — 챕터 주제와 맞을 때만).

## 7. 내용 작성 원칙

1. 강의자료(PPT)의 **모든 개념과 코드 예제**를 빠짐없이 옮깁니다 (설치 화면 캡처 설명 등은 요약).
   그 위에 6-2의 기준만큼 내용을 채웁니다 — 강의자료만 옮긴 분량은 부족합니다.
