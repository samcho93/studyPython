...
(function () {
  ...
  PY_COURSE.addChapter({
    ...
    goals: [
      ...
      'f-문자열 서식(정렬 · 천 단위 쉼표 · 백분율 · 진수)으로 표 모양의 출력을 만들 수 있다',
      '변수가 값에 붙인 이름표임을 이해하고, 실수의 오차를 round · math.isclose · decimal 로 다룰 수 있다',
    ...
    sections: [
      /* ───────────────────────── 1교시: print() 서식 ───────────────────────── */
      {
        ...
        content: [
          ...
          { type: 'h', text: '한 걸음 더: %% 와 %r, 그리고 반올림 규칙' },
          { type: 'p', html: '서식 문자열 안에서 <b>퍼센트 기호 자체</b>를 출력하려면 <code>%%</code> 처럼 두 번 써야 합니다. 한 번만 쓰면 파이썬은 “서식이 시작되는구나” 하고 다음 글자를 기다리다가 오류를 냅니다. 또 <code>%s</code> 와 비슷하지만 <b>따옴표까지 보여 주는</b> <code>%r</code> 도 있습니다. <code>%r</code> 은 사람이 읽기 좋은 모양이 아니라 <b>“파이썬이 보는 모양”</b>을 그대로 보여 주므로, 값이 문자열인지 숫자인지 확인할 때 아주 유용합니다.' },
          { type: 'code', title: '추가 예제. 퍼센트 기호 · 반올림 · %r', code: 'print("할인율 %d%%" % 30)\nprint("%.0f %.0f %.0f %.0f" % (0.5, 1.5, 2.5, 3.5))\nprint("%.2f" % 2.675)\nprint("[%s] 와 [%r]" % ("100", "100"))\nprint("[%s] 와 [%r]" % (100, 100))',
            expect: '할인율 30%\n0 2 2 4\n2.67\n[100] 와 [\'100\']\n[100] 와 [100]',
            desc: '<code>2행</code>의 결과가 <code>1 2 3 4</code> 가 아니라 <code>0 2 2 4</code> 인 점에 주목하세요. <code>3행</code>도 2.68 이 아니라 2.67 입니다. <code>4~5행</code> <code>%r</code> 은 문자열이면 따옴표를 붙여 보여 주므로, <b>겉보기가 같은 100 과 "100" 을 구별</b>할 수 있습니다.' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: 0.5 가 0 으로 반올림되는 까닭', html: '<p>학교에서 배운 “5 이상이면 올림”은 같은 숫자를 여러 번 더할 때 결과가 조금씩 커지는 쪽으로 치우칩니다. 그래서 파이썬을 비롯한 대부분의 언어는 <b>끝자리가 딱 5 일 때 가까운 짝수 쪽으로</b> 맞추는 방식(round half to even, 은행가 반올림)을 씁니다. 0.5 → 0, 1.5 → 2, 2.5 → 2, 3.5 → 4 가 되는 이유입니다.</p><p><code>2.675</code> 가 2.67 이 되는 것은 또 다른 이유입니다. 컴퓨터에 저장된 값이 사실은 2.67499…99 여서, 규칙대로 내림한 것입니다. 실수의 오차는 5교시에서 자세히 다룹니다. <b>돈처럼 정확해야 하는 계산</b>은 <code>decimal</code> 모듈을 쓴다는 것만 기억해 두세요.</p>' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: % 방식 · format() · f-문자열 — 무엇을 쓸까', html: '<p>파이썬에는 출력 서식을 지정하는 방법이 세 가지 있습니다. <code>%</code> 방식은 C 언어에서 온 가장 오래된 방법이고, <code>"{}".format()</code> 은 그다음, <b>f-문자열</b>(<code>f"…"</code>)이 가장 새롭고 요즘 실무에서 가장 많이 쓰입니다.</p><p>이 강좌는 강의자료를 따라 <code>%</code> 방식으로 서식의 원리를 익힌 뒤, 2교시에서 <code>format()</code> 과 f-문자열을 정리합니다. <b>자릿수 · 정렬 규칙은 세 방식이 거의 같으므로</b>, 지금 배우는 <code>%5d</code>, <code>%7.2f</code> 의 감각이 그대로 이어집니다.</p>' }
        ],
        practice: [
          ...
          { title: '실습 3-2. 오른쪽 끝을 맞춘 숫자', level: 2,
            ...
            solution: 'print("%6d" % 7)\nprint("%6d" % 65)\nprint("%6d" % 432)\nprint("%6d" % 1024)\nprint("%06d" % 1024)\n',
            expect: '     7\n    65\n   432\n  1024\n001024' },
          { title: '실습 3-3. 퍼센트와 소수점', level: 1,
            desc: '<p>다음 세 줄을 서식 문자로 출력하세요. 퍼센트 기호는 <code>%%</code> 로 씁니다.</p><ul><li>정답률: 87.456 을 <b>소수점 아래 한 자리</b> + <code>%</code></li><li>할인율: 30 을 <b>정수</b> + <code>%</code></li><li>잔액: 12345.678 을 <b>전체 10칸, 소수점 아래 두 자리</b></li></ul><pre>정답률 : 87.5%\n할인율 : 30%\n잔액 :   12345.68원</pre>',
            hint: '<code>"%.1f%%"</code>, <code>"%d%%"</code>, <code>"%10.2f"</code> 를 사용합니다. <code>%</code> 를 한 번만 쓰면 오류가 납니다.',
            starter: '# 서식 문자로 세 줄을 출력하세요\nprint("정답률 : %.1f%%" % 87.456)\n# TODO: 할인율 (정수 + %)\n# TODO: 잔액 (전체 10칸, 소수점 아래 2자리)\n',
            solution: 'print("정답률 : %.1f%%" % 87.456)\nprint("할인율 : %d%%" % 30)\nprint("잔액 : %10.2f원" % 12345.678)\n',
            expect: '정답률 : 87.5%\n할인율 : 30%\n잔액 :   12345.68원' },
          { title: '실습 3-4. 영수증 줄 맞추기', level: 2,
            desc: '<p>품목은 <b>왼쪽 정렬 10칸</b>, 수량은 <b>오른쪽 정렬 5칸</b>, 금액은 <b>오른쪽 정렬 10칸에 소수점 아래 두 자리</b>로 출력하세요. 첫 줄의 제목도 같은 폭으로 맞춥니다.</p><pre>item        qty     total\ncoffee        2      9.00\nsandwich      1     12.50\ncookie        3      4.50</pre>',
            hint: '제목 줄은 <code>"%-10s%5s%10s"</code>, 자료 줄은 <code>"%-10s%5d%10.2f"</code> 입니다. 한글은 화면 폭이 달라 줄이 어긋날 수 있어 영어를 씁니다.',
            starter: 'print("%-10s%5s%10s" % ("item", "qty", "total"))\nprint("%-10s%5d%10.2f" % ("coffee", 2, 9.0))\n# TODO: sandwich 1개 12.5, cookie 3개 4.5 줄을 추가하세요\n',
            solution: 'print("%-10s%5s%10s" % ("item", "qty", "total"))\nprint("%-10s%5d%10.2f" % ("coffee", 2, 9.0))\nprint("%-10s%5d%10.2f" % ("sandwich", 1, 12.5))\nprint("%-10s%5d%10.2f" % ("cookie", 3, 4.5))\n',
            expect: 'item        qty     total\ncoffee        2      9.00\nsandwich      1     12.50\ncookie        3      4.50' },
          { title: '실습 3-5. 서식으로 명함 상자 그리기', level: 3,
            desc: '<p>안쪽 폭이 <b>24칸</b>인 상자를 서식 문자만으로 그리세요. 가운데 정렬은 <code>%</code> 서식에 없으므로 <b>앞쪽 빈칸 수를 직접 계산</b>해야 합니다. (<code>Python Beginner</code> 는 15글자 → 24 − 15 = 9, 앞 4칸 · 뒤 5칸)</p><pre>+------------------------+\n|Hong Gildong            |\n|    Python Beginner     |\n|             hong@py.org|\n+------------------------+</pre><p>여기까지 했다면 이름을 자기 이름(영문)으로 바꾸고, 가운데 줄의 글자 수가 달라질 때 앞 빈칸 수가 어떻게 바뀌어야 하는지 계산해 보세요.</p>',
            hint: '왼쪽 정렬은 <code>"|%-24s|"</code>, 오른쪽 정렬은 <code>"|%24s|"</code>. 가운데 정렬은 <code>"|%-24s|" % "    Python Beginner"</code> 처럼 앞에 빈칸을 직접 붙입니다.',
            starter: 'print("+------------------------+")\nprint("|%-24s|" % "Hong Gildong")\n# TODO: 가운데 정렬 줄 (앞 4칸 빈칸)\n# TODO: 오른쪽 정렬 줄\nprint("+------------------------+")\n',
            solution: 'print("+------------------------+")\nprint("|%-24s|" % "Hong Gildong")\nprint("|%-24s|" % "    Python Beginner")\nprint("|%24s|" % "hong@py.org")\nprint("+------------------------+")\n',
            expect: '+------------------------+\n|Hong Gildong            |\n|    Python Beginner     |\n|             hong@py.org|\n+------------------------+' }
        ],
        quiz: [
          ...
          { q: '<b>오류 찾기</b> — 다음 코드는 왜 <code>ValueError</code> 가 날까?<pre><code>print("성공률 %d%" % 80)</code></pre>', options: ['80 은 정수가 아니라서', '<code>%</code> 기호를 출력하려면 <code>%%</code> 로 써야 하는데 한 번만 써서', '<code>%d</code> 는 퍼센트와 함께 쓸 수 없어서', '값을 괄호로 묶지 않아서'], answer: 1,
            explain: '마지막 <code>%</code> 뒤에 서식 글자가 없어 파이썬이 서식이 끝나지 않았다고 판단합니다. <code>"성공률 %d%%"</code> 로 고치면 <code>성공률 80%</code> 가 출력됩니다.' }
        ],
        slides: [
          ...
          { layout: 'two', title: '오른쪽 정렬 vs 왼쪽 정렬',
            left: { title: '기본: 오른쪽 정렬', code: 'print("%8s|" % "Python")\nprint("%8d|" % 123)\nprint("%8.2f|" % 3.14159)' },
            right: { title: '- 를 붙이면 왼쪽 정렬', code: 'print("%-8s|" % "Python")\nprint("%-8d|" % 123)\nprint("%-8.2f|" % 3.14159)' },
            notes: '<p>세로 막대(|)를 함께 출력하면 빈칸이 어디에 붙는지 눈에 보입니다. 표를 만들 때 <b>글자는 왼쪽, 숫자는 오른쪽</b>이 읽기 좋다는 실무 감각을 알려 줍니다.</p><p><b>발문</b>: “값이 8칸보다 길면 잘릴까요?” → 잘리지 않고 그대로 나옵니다(폭 = 최소 칸 수).</p><p>시간: 4분</p>' },
          { layout: 'code', title: '한 걸음 더: %% · 반올림 · %r', code: 'print("할인율 %d%%" % 30)\nprint("%.0f %.0f %.0f %.0f" % (0.5, 1.5, 2.5, 3.5))\nprint("%.2f" % 2.675)\nprint("[%s] 와 [%r]" % ("100", "100"))',
            points: ['%% → 퍼센트 기호 자체', '끝자리 5 는 가까운 짝수로 (0 2 2 4)', '2.675 → 2.67 (실수 오차, 5교시)', '%r 은 따옴표까지 — 문자열/숫자 구별'],
            notes: '<p>2행 결과를 먼저 예측하게 하면 거의 모두 <code>1 2 3 4</code> 라고 답합니다. “은행가 반올림”을 소개하며 여러 번 더할 때의 치우침을 설명합니다.</p><p><code>%r</code> 은 디버깅에 아주 유용합니다 — 눈에 보이는 100 이 숫자인지 문자열인지 확인할 수 있습니다.</p><p>시간: 5분</p>' },
      ...
      /* ───────────────────────── 2교시: format() · 이스케이프 · 프로그램 1 ───────────────────────── */
      {
        ...
        goals: [
          ...
          'f-문자열의 정렬(&lt; &gt; ^) · 천 단위 쉼표 · 백분율 · 채움 문자 서식을 사용할 수 있다',
        ...
        content: [
          ...
          { type: 'h', text: 'f-문자열 서식 총정리 (중급)' },
          { type: 'p', html: 'f-문자열의 중괄호는 <code>{값 <b>!변환</b> : <b>서식</b>}</code> 세 부분으로 이루어집니다. 서식 부분은 <code>%</code> 방식에서 배운 <b>폭 · 소수 자릿수</b>에 몇 가지가 더해진 것인데, 특히 <b>정렬 기호(<code>&lt; &gt; ^</code>)</b>와 <b>천 단위 쉼표(<code>,</code>)</b>는 % 방식에는 없는 기능이라 실무에서 f-문자열을 선호하는 큰 이유가 됩니다.' },
          { type: 'table', head: ['서식', '뜻', '예', '결과 (□ = 빈칸)'], rows: [
            ['<code>:.2f</code>', '소수점 아래 2자리', '<code>f"{3.14159:.2f}"</code>', '<code>3.14</code>'],
            ['<code>:,</code>', '천 단위마다 쉼표', '<code>f"{1234567:,}"</code>', '<code>1,234,567</code>'],
            ['<code>:&gt;10</code>', '10칸 오른쪽 정렬', '<code>f"{\'Py\':&gt;10}"</code>', '<code>□□□□□□□□Py</code>'],
            ['<code>:&lt;10</code>', '10칸 왼쪽 정렬', '<code>f"{\'Py\':&lt;10}"</code>', '<code>Py□□□□□□□□</code>'],
            ['<code>:^10</code>', '10칸 가운데 정렬', '<code>f"{\'Py\':^10}"</code>', '<code>□□□□Py□□□□</code>'],
            ['<code>:08.3f</code>', '전체 8칸, 소수 3자리, 0 채움', '<code>f"{3.5:08.3f}"</code>', '<code>0003.500</code>'],
            ['<code>:.1%</code>', '백분율로 (×100 후 %)', '<code>f"{0.25:.1%}"</code>', '<code>25.0%</code>'],
            ['<code>:*^11</code>', '채움 문자 지정 + 가운데', '<code>f"{42:*^11}"</code>', '<code>****42*****</code>'],
            ['<code>:+.1f</code>', '양수에도 부호 표시', '<code>f"{3.5:+.1f}"</code>', '<code>+3.5</code>'],
            ['<code>!r</code>', '따옴표까지 보이는 모양', '<code>f"{\'Py\'!r}"</code>', '<code>\'Py\'</code>'],
            ['<code>=</code>', '식과 값을 함께', '<code>f"{2+3=}"</code>', '<code>2+3=5</code>']
          ], caption: '표 3-2-1. 자주 쓰는 f-문자열 서식 (서식 규칙은 format() 에서도 그대로 사용)' },
          { type: 'code', title: '추가 예제. f-문자열 서식 한눈에 보기', code: 'print(f"{3.14159:.2f}")\nprint(f"{1234567:,}")\nprint(f"{1234567.891:,.2f}")\nprint(f"[{\'Python\':>10}]")\nprint(f"[{\'Python\':<10}]")\nprint(f"[{\'Python\':^10}]")\nprint(f"{3.5:08.3f}")\nprint(f"{0.25:.1%}")',
            expect: '3.14\n1,234,567\n1,234,567.89\n[    Python]\n[Python    ]\n[  Python  ]\n0003.500\n25.0%',
            desc: '<code>2~3행</code> <code>:,</code> 하나로 금액에 쉼표가 붙습니다. <code>4~6행</code> <code>&gt;</code> 오른쪽, <code>&lt;</code> 왼쪽, <code>^</code> 가운데 정렬입니다. 가운데 정렬은 <code>%</code> 방식에는 없어 직접 빈칸을 계산해야 했던 기능입니다.' },
          { type: 'code', title: '추가 예제. 채움 문자 · 부호 · 진수 서식', code: 'print(f"[{42:*^11}]")\nprint(f"[{42:0>7}]")\nprint(f"{3.5:+.1f} {-3.5:+.1f}")\nprint(f"[{255:8x}] [{255:8X}] [{255:#8x}]")',
            expect: '[****42*****]\n[0000042]\n+3.5 -3.5\n[      ff] [      FF] [    0xff]',
            desc: '정렬 기호 <b>앞</b>에 글자를 하나 쓰면 <b>채움 문자</b>가 됩니다(<code>*^11</code> = <code>*</code> 로 채우며 11칸 가운데). <code>+</code> 는 양수에도 부호를 붙여 증감표를 만들 때 좋습니다. <code>x</code> · <code>X</code> · <code>#x</code> 는 16진수 표기로, 4교시에서 다시 만납니다.' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: f"{값=}" 와 !r — 디버깅이 편해진다', html: '<p>중괄호 안에 <code>=</code> 를 붙이면 <b>식과 결과를 함께</b> 출력합니다: <code>f"{score=}"</code> → <code>score=92.5</code>. <code>print("score =", score)</code> 처럼 이름을 두 번 쓰지 않아도 되고, 이름을 바꿔도 출력이 자동으로 따라오므로 <b>값을 확인하는 print</b> 에 아주 좋습니다.</p><p><code>!r</code> 은 1교시의 <code>%r</code> 과 같습니다. 문자열이면 따옴표를 붙여 보여 주므로 <b>눈에 보이지 않는 공백이나 숫자/문자열 혼동</b>을 잡아낼 수 있습니다. <code>f"{name!r}"</code> 이 <code>\'10 \'</code> 처럼 나오면 뒤에 공백이 붙어 있다는 뜻입니다.</p>' },
          { type: 'code', title: '추가 예제. = 와 !r 로 값 확인하기', code: 'name = "홍길동"\nscore = 92.567\nprint(f"{score=}")\nprint(f"{score=:.1f}")\nprint(f"{name!r} 님의 점수는 {score:6.2f}점")\nprint(f"{2 + 3 * 4=}")',
            expect: "score=92.567\nscore=92.6\n'홍길동' 님의 점수는  92.57점\n2 + 3 * 4=14",
            desc: '변수는 다음 시간(3교시)에 배우지만, f-문자열의 진짜 힘은 변수와 함께 쓸 때 나오므로 미리 사용했습니다. <code>6행</code>처럼 계산식도 그대로 넣을 수 있어 연산자 우선순위를 확인할 때 편리합니다.' },
...
print(r"C:\new\test")`, expect: 'C:\new\test\n' +String.raw`C:\new\test` + '\n' + String.raw`C:\new\test`,
          ...
          { type: 'h', text: '한 걸음 더: 유니코드 문자와 긴 줄 이어 쓰기' },
          { type: 'p', html: '이스케이프 문자는 <b>키보드로 치기 어려운 글자</b>를 쓸 때도 사용합니다. <code>\\uXXXX</code> 는 유니코드 번호(16진수 4자리), <code>\\xXX</code> 는 1바이트 문자 코드입니다. 또 코드가 너무 길어질 때 <b>줄 끝에 역슬래시</b>를 붙이면 다음 줄과 한 줄로 이어집니다.' },
          { type: 'code', title: '추가 예제. \\u · \\x 와 줄 이어 쓰기', code: String.raw`print("\u00b0C", "\u2764", "\x41\x42")
print("이 문장은 코드에서 두 줄이지만 \
출력은 한 줄입니다")
print("C:\\temp\\new" + "\\")`,
            expect: '°C ❤ AB\n이 문장은 코드에서 두 줄이지만 출력은 한 줄입니다\nC:\\temp\\new\\',
            desc: '<code>1행</code> <code>\\u00b0</code> 는 도(°), <code>\\u2764</code> 는 하트(❤), <code>\\x41</code> 은 A 입니다. <code>2~3행</code> 줄 끝의 역슬래시 뒤에는 <b>공백조차 있으면 안 됩니다</b>. <code>4행</code>처럼 문자열을 <code>+</code> 로 이어 붙일 수도 있습니다.' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: raw 문자열은 역슬래시로 끝날 수 없다', html: '<p><code>r"C:\\temp\\"</code> 는 <code>SyntaxError</code> 입니다. raw 문자열에서도 역슬래시는 <b>다음 따옴표를 글자로 만드는 역할</b>은 그대로 하기 때문에, 문자열이 끝나지 않은 것으로 보입니다. 폴더 경로를 역슬래시로 끝내야 한다면 <code>r"C:\\temp" + "\\\\"</code> 처럼 나누어 쓰거나, 아예 <code>"C:/temp/"</code> 처럼 <b>슬래시</b>를 쓰면 됩니다. 윈도우도 파이썬 코드 안에서는 슬래시 경로를 잘 알아듣습니다.</p><p>경로를 많이 다루게 되면 <code>pathlib</code> 모듈(<code>Path("C:/temp") / "data.txt"</code>)이 훨씬 안전합니다. 파일 입출력 장에서 다시 만납니다.</p>' },
        ...
        practice: [
          { title: 'SELF STUDY 3-1. 모래시계 모양 출력', level: 1,
            ...
            solution: 'print("*********")\nprint(" ******* ")\nprint("  *****  ")\nprint("   ***   ")\nprint("    *    ")\nprint("   ***   ")\nprint("  *****  ")\nprint(" ******* ")\nprint("*********")\n',
            expect: '*********\n *******\n  *****\n   ***\n    *\n   ***\n  *****\n *******\n*********' },
          { title: '실습 3-6. 이스케이프 문자 연습', level: 1,
            desc: '<p>다음 세 줄을 출력하세요. 첫 줄의 큰따옴표와 탭, 둘째 줄의 작은따옴표, 셋째 줄의 역슬래시에 주의합니다.</p><pre>"파이썬"\tPython\n그는 \'OK\' 라고 말했다.\nC:\\temp\\new</pre>',
            hint: '큰따옴표로 감싼 문자열 안의 큰따옴표는 <code>\\"</code>, 탭은 <code>\\t</code> 입니다. 셋째 줄은 <code>r"…"</code> 이나 <code>"C:\\\\temp\\\\new"</code> 로 씁니다.',
            starter: String.raw`print("\"파이썬\"\tPython")
# TODO: 둘째 줄 (작은따옴표 포함)
# TODO: 셋째 줄 (역슬래시 포함)
`,
            solution: String.raw`print("\"파이썬\"\tPython")
print("그는 'OK' 라고 말했다.")
print(r"C:\temp\new")
`,
            expect: '"파이썬"\tPython\n그는 \'OK\' 라고 말했다.\nC:\\temp\\new' },
          { title: '실습 3-7. print() 한 번으로 성적표 출력', level: 2,
            ...
            expect: '이름\t국어\t수학\n철수\t90\t85\n영희\t100\t95\n"과목"은 2개' },
          { title: '실습 3-8. format() 으로 순서 바꾸기', level: 2,
            ...
            expect: '체리 - 바나나 - 사과\n사과 사과 사과' },
          { title: '실습 3-9. f-문자열로 가격표 만들기', level: 2,
            desc: '<p>f-문자열의 <b>정렬(<code>&lt;</code> <code>&gt;</code>)</b>과 <b>천 단위 쉼표(<code>,</code>)</b>를 사용해 가격표를 출력하세요. 품목은 왼쪽 10칸, 단가는 오른쪽 9칸(쉼표 포함), 수량은 오른쪽 6칸입니다. 마지막 줄의 합계 금액은 <b>식을 직접 중괄호 안에 써서</b> 계산하세요.</p><pre>item          price   qty\ncoffee        3,500     2\ncake         12,000     1\ntotal        19,000     3</pre>',
            hint: '<code>f"{\'item\':&lt;10}{\'price\':&gt;9}{\'qty\':&gt;6}"</code> 처럼 제목 줄도 같은 폭으로 맞춥니다. 숫자 줄은 <code>f"{\'coffee\':&lt;10}{3500:&gt;9,}{2:&gt;6}"</code>, 합계는 <code>{3500 * 2 + 12000:&gt;9,}</code>.',
            starter: `print(f"{'item':<10}{'price':>9}{'qty':>6}")
print(f"{'coffee':<10}{3500:>9,}{2:>6}")
# TODO: cake 12000원 1개 줄
# TODO: total 줄 (금액과 수량은 식으로 계산)
`,
            solution: `print(f"{'item':<10}{'price':>9}{'qty':>6}")
print(f"{'coffee':<10}{3500:>9,}{2:>6}")
print(f"{'cake':<10}{12000:>9,}{1:>6}")
print(f"{'total':<10}{3500 * 2 + 12000:>9,}{2 + 1:>6}")
`,
            expect: 'item          price   qty\ncoffee        3,500     2\ncake         12,000     1\ntotal        19,000     3' },
          { title: '실습 3-10. 진행 막대(progress bar) 그리기', level: 3,
            desc: '<p>작업 진행률을 아래 모양으로 출력하세요. 막대는 전체 <b>10칸</b>이며 진행한 만큼 <code>#</code>, 남은 만큼 <code>-</code> 를 채웁니다. 퍼센트는 <b>전체 6칸, 소수점 아래 한 자리</b>의 백분율 서식(<code>:6.1%</code>)으로 출력합니다.</p><pre>[----------]   0.0%\n[###-------]  30.0%\n[#######---]  70.0%\n[##########] 100.0%</pre><p>여기까지 했다면 막대 길이를 20칸으로 바꿔 보고, 55% 줄도 추가해 보세요. (55% 는 <code>#</code> 이 몇 개일까요?)</p>',
            hint: '<code>"#" * 3</code> 은 <code>###</code> 입니다. f-문자열 중괄호 안에 <code>{\'#\' * 3 + \'-\' * 7}</code> 처럼 식을 그대로 쓸 수 있습니다. 백분율 서식 <code>:6.1%</code> 에는 0.3 처럼 <b>1 이하의 값</b>을 넣습니다.',
            starter: `print(f"[{'#' * 0 + '-' * 10}] {0.0:6.1%}")
# TODO: 30%, 70%, 100% 줄을 추가하세요
`,
            solution: `print(f"[{'#' * 0 + '-' * 10}] {0.0:6.1%}")
print(f"[{'#' * 3 + '-' * 7}] {0.3:6.1%}")
print(f"[{'#' * 7 + '-' * 3}] {0.7:6.1%}")
print(f"[{'#' * 10 + '-' * 0}] {1.0:6.1%}")
`,
            expect: '[----------]   0.0%\n[###-------]  30.0%\n[#######---]  70.0%\n[##########] 100.0%' }
        ],
        quiz: [
          ...
          { q: '다음 코드의 실행 결과는?<pre><code>print(f"[{1234567:,}]")\nprint(f"[{\'Py\':^6}]")</code></pre>', options: ['[1234567] / [Py]', '[1,234,567] / [□□Py□□]', '[1,234,567] / [Py□□□□]', '[1234567] / [□□□□Py]'], answer: 1,
            explain: '<code>:,</code> 는 천 단위 쉼표, <code>:^6</code> 은 6칸 가운데 정렬입니다. 남는 4칸이 앞뒤로 2칸씩 나뉩니다. (□는 빈칸)' }
        ],
        slides: [
          ...
          { layout: 'table', title: 'f-문자열 서식 총정리 (중급)', head: ['서식', '뜻', '결과 (□ = 빈칸)'], rows: [
            [':.2f', '소수점 아래 2자리', '3.14'], [':,', '천 단위 쉼표', '1,234,567'], [':>10 / :<10 / :^10', '오른쪽 / 왼쪽 / 가운데 정렬', '□□□□□□□□Py / Py□□□□□□□□ / □□□□Py□□□□'],
            [':08.3f', '전체 8칸 · 소수 3자리 · 0 채움', '0003.500'], [':.1%', '백분율', '25.0%'], [':*^11', '채움 문자 + 가운데', '****42*****'], ['!r', '따옴표까지 보이는 모양', "'Py'"]],
            lead: '{값 !변환 : 서식} — 서식 규칙은 format() 과 동일',
            notes: '<p><b>정렬 기호와 천 단위 쉼표는 % 방식에 없습니다.</b> 실무에서 f-문자열을 쓰는 큰 이유라고 알려 줍니다.</p><p>가운데 정렬(<code>^</code>)은 1교시에서 빈칸을 직접 세어 만들었던 것을 한 글자로 해결해 줍니다.</p><p>시간: 4분</p>' },
          { layout: 'code', title: 'f-문자열 서식 실행해 보기', code: 'print(f"{1234567.891:,.2f}")\nprint(f"[{\'Python\':>10}]")\nprint(f"[{\'Python\':^10}]")\nprint(f"{3.5:08.3f}")\nprint(f"[{42:*^11}]")\nprint(f"{0.25:.1%}")',
            points: [':,.2f → 쉼표 + 소수 2자리', '> 오른쪽 · < 왼쪽 · ^ 가운데', '채움 문자는 정렬 기호 앞에', ':.1% → ×100 후 % 붙임'],
            notes: '<p>한 줄씩 값을 바꿔 가며 실행해 보여 줍니다. 특히 <code>:.1%</code> 에는 0.25 처럼 <b>1 이하의 값</b>을 넣어야 한다는 점을 짚습니다(25 를 넣으면 2500.0%).</p><p>시간: 4분</p>' },
          { layout: 'code', title: '디버깅에 좋은 f"{값=}" 와 !r', code: 'name = "홍길동"\nscore = 92.567\nprint(f"{score=}")\nprint(f"{score=:.1f}")\nprint(f"{name!r} 님의 점수")\nprint(f"{2 + 3 * 4=}")',
            points: ['{값=} → 식과 결과를 함께 출력', '이름을 두 번 쓰지 않아도 된다', '!r → 따옴표까지 (공백·형 확인)', '중괄호 안에 계산식도 가능'],
            notes: '<p>변수는 다음 시간에 배우지만 여기서 미리 맛봅니다. “print 로 값 확인하기”는 앞으로 가장 많이 쓸 디버깅 방법이므로 꼭 시연하세요.</p><p><b>발문</b>: “<code>f\"{name!r}\"</code> 이 <code>\'10 \'</code> 로 나왔다면 무엇을 알 수 있을까요?” → 숫자가 아니라 문자열이고, 뒤에 공백이 있다.</p><p>시간: 4분</p>' },
...
print(r"\n \t \" \\를 그대로 출력")`,
          ...
          { layout: 'practice', title: 'SELF STUDY 3-1. 모래시계 모양', desc: '별표가 9 → 7 → 5 → 3 → 1 → 3 → 5 → 7 → 9 개가 되도록 가운데 정렬해 출력하세요.',
            starter: 'print("*********")\nprint(" ******* ")\n# TODO: 나머지 줄을 완성하세요\n',
            solution: 'print("*********")\nprint(" ******* ")\nprint("  *****  ")\nprint("   ***   ")\nprint("    *    ")\nprint("   ***   ")\nprint("  *****  ")\nprint(" ******* ")\nprint("*********")\n',
            notes: '<p>빨리 끝낸 학생에게는 실습 3-7(print 한 번으로 성적표)이나 실습 3-10(진행 막대)을 추가 과제로 줍니다.</p><p>시간: 8분</p>' },
          { layout: 'summary', title: '정리', bullets: ['"{0:5d}".format(값) — 번호(0부터)와 서식', 'format() 은 번호로 출력 순서를 바꿀 수 있다', '(중급) f"{값:&gt;10}" · {값:,} · {값:.1%} · {값=} · {값!r}', '이스케이프: \\n 줄 바꿈, \\t 탭, \\\\ \\\' \\" 글자 출력', 'r"…" raw 문자열은 이스케이프를 해석하지 않는다 (끝에 \\ 는 ✘)', '[프로그램 1] 빈칸 + 별로 가운데 정렬'],
      ...
      /* ───────────────────────── 3교시: 변수 ───────────────────────── */
      {
        ...
        goals: [
          ...
          '변수가 값에 붙인 이름표임을 id() · is 로 확인하고, 문자열이 불변임을 설명할 수 있다',
          'PEP 8 이름 규칙에 맞게 변수 · 상수 이름을 지을 수 있다'
        ],
        ...
        content: [
          ...
          { type: 'h', text: '한 걸음 더: 이름표 모델을 id() 로 확인하기' },
          { type: 'p', html: '“그릇” 비유는 배우기 쉽지만, 파이썬이 실제로 하는 일과는 조금 다릅니다. 파이썬은 값(객체)을 메모리 어딘가에 만들어 두고 <b>변수 이름을 그 값에 붙여 둡니다</b>. 값이 저장된 위치를 확인하는 함수가 <code>id()</code> 이고, <b>두 이름이 같은 값을 가리키는지</b> 묻는 연산자가 <code>is</code> 입니다.' },
          { type: 'code', title: '추가 예제. id() 와 is 로 이름표 확인하기', code: 'a = 100\nb = a\nprint("처음:", a is b, id(a) == id(b))\n\nb = 200        # b 이름표만 새 값으로 옮겨 간다\nprint("대입 후:", a, b, a is b)\n\nc = 200\nprint("값 비교:", b == c)',
            expect: '처음: True True\n대입 후: 100 200 False\n값 비교: True',
            desc: '<code>3행</code> <code>b = a</code> 직후에는 두 이름이 <b>같은 값</b>을 가리킵니다. <code>5행</code>에서 b 에 새 값을 대입해도 a 는 그대로입니다. 그릇 비유로 “값을 복사했다”고 설명한 것과 결과가 같지요. <code>8행</code> <b>값이 같은지</b> 묻는 것은 <code>is</code> 가 아니라 <code>==</code> 입니다.' },
          { type: 'callout', kind: 'warn', title: 'is 와 == 를 헷갈리지 마세요', html: '<code>is</code> 는 <b>“같은 값(객체)인가”</b>, <code>==</code> 는 <b>“값이 같은가”</b>입니다. 사람으로 치면 <code>is</code> 는 “같은 사람인가”, <code>==</code> 는 “이름이 같은가”에 해당합니다. 값을 비교할 때는 항상 <code>==</code> 를 쓰세요. <code>is</code> 를 쓰는 거의 유일한 경우는 <code>x is None</code> 입니다(5교시).' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: 이름표가 사라진 값은 어떻게 될까 (가비지 컬렉션)', html: '<p><code>a = 100</code> 뒤에 <code>a = 200</code> 을 하면, 100 이라는 값에는 더 이상 붙은 이름표가 없습니다. 파이썬은 <b>아무도 가리키지 않는 값을 자동으로 정리</b>해 메모리를 되돌려 줍니다. 이것을 <b>가비지 컬렉션(garbage collection)</b> 이라고 합니다.</p><p>C 언어에서는 프로그래머가 직접 메모리를 반납해야 하고, 잊어버리면 <b>메모리 누수</b>가 생깁니다. 파이썬은 이 일을 대신 해 주기 때문에 초보자도 안심하고 변수를 만들 수 있습니다. 다만 “자동이라서 신경 쓸 필요가 없다”가 아니라 <b>“쓰지 않는 값은 알아서 치워진다”</b> 정도로 이해해 두세요.</p>' },

          { type: 'h', text: '한 걸음 더: 문자열은 바꿀 수 없다 (불변, immutable)' },
          { type: 'p', html: '숫자와 문자열은 <b>한 번 만들어지면 내용을 바꿀 수 없는</b> 값입니다. 이것을 <b>불변(immutable)</b> 이라고 합니다. 문자열을 “고치는” 것처럼 보이는 코드는 사실 <b>새 문자열을 만들어 이름표를 옮기는</b> 것입니다.' },
          { type: 'code', title: '추가 예제. 문자열은 새로 만들어진다', code: 's = "python"\nt = s\ns = s + "!"          # 새 문자열 "python!" 이 만들어지고 s 가 그것을 가리킨다\nprint(s, t)\n\nprint(t.upper(), t)  # upper() 도 원본을 바꾸지 않고 새 문자열을 돌려준다',
            expect: 'python! python\nPYTHON python',
            desc: '<code>4행</code> t 는 여전히 원래 문자열을 가리키므로 <code>python</code> 입니다. <code>6행</code> <code>upper()</code> 는 대문자로 바꾼 <b>새 문자열</b>을 돌려줄 뿐, t 자체는 그대로입니다. 바꾸고 싶다면 <code>t = t.upper()</code> 처럼 <b>다시 대입</b>해야 합니다.' },
          { type: 'code', title: '추가 예제. 문자열의 글자 하나만 바꾸려 하면 (TypeError)', code: 's = "python"\ns[0] = "P"', expectError: true,
            expect: 'Traceback (most recent call last):\n  File "main.py", line 2, in <module>\n    s[0] = "P"\n    ~^^^\nTypeError: \'str\' object does not support item assignment',
            desc: '“문자열 객체는 항목 대입을 지원하지 않는다”는 뜻입니다. 첫 글자만 대문자로 바꾸려면 <code>s = "P" + s[1:]</code> 나 <code>s = s.capitalize()</code> 처럼 <b>새 문자열을 만들어 다시 대입</b>합니다. (문자열 다루기는 뒤 장에서 자세히 배웁니다)' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: 왜 불변이 좋을까', html: '값이 절대 바뀌지 않으면, 여러 이름이 같은 값을 가리켜도 <b>한쪽에서 몰래 바꿔 다른 쪽이 깨지는 일</b>이 없습니다. 그래서 숫자 · 문자열은 안심하고 주고받을 수 있습니다. 반대로 <b>가변(mutable)</b>인 리스트(8장)는 <code>b = a</code> 뒤에 <code>b</code> 를 바꾸면 <code>a</code> 도 함께 바뀌어 초보자가 가장 많이 놀라는 부분입니다. 오늘 배운 “이름표” 모델을 기억해 두면 그때 훨씬 쉽게 이해할 수 있습니다.' },

          { type: 'h', text: '변수와 f-문자열, 그리고 상수 표기' },
          { type: 'p', html: '2교시에 배운 f-문자열은 <b>변수와 함께 쓸 때</b> 진가를 발휘합니다. 변수 이름을 중괄호 안에 그대로 쓰면 되므로, 값의 순서를 헷갈릴 일이 없습니다.' },
          { type: 'code', title: '추가 예제. 변수를 f-문자열로 출력하기', code: 'name = "홍길동"\nkor = 92\neng = 87.5\ntotal = kor + eng\n\nprint(f"{name:>6} | 국어 {kor:3d} | 영어 {eng:5.1f} | 합계 {total:6.1f}")\nprint(f"{name} 의 평균: {total / 2:.2f}점")\nprint(f"{total=}")',
            expect: '   홍길동 | 국어  92 | 영어  87.5 | 합계  179.5\n홍길동 의 평균: 89.75점\ntotal=179.5',
            desc: '같은 줄을 <code>%</code> 방식으로 쓰면 값의 순서를 일일이 맞춰야 하지만, f-문자열은 <b>보이는 자리에 변수 이름이 그대로</b> 있어 읽기 쉽습니다.' },
          { type: 'code', title: '추가 예제. 바뀌지 않을 값은 대문자 이름으로 (상수)', code: 'PI = 3.141592\nMAX_SCORE = 100\n\nradius = 3\nprint(f"반지름 {radius} 인 원의 넓이: {PI * radius ** 2:.2f}")\nprint(f"88점의 비율: {88 / MAX_SCORE:.1%}")',
            expect: '반지름 3 인 원의 넓이: 28.27\n88점의 비율: 88.0%',
            desc: '파이썬에는 “절대 못 바꾸는 상수” 문법이 없습니다. 대신 <b>모두 대문자 + 언더바</b>로 이름을 지어 “이 값은 바꾸지 말자”고 약속합니다. 코드 곳곳에 흩어진 <code>3.141592</code> 나 <code>100</code> 같은 숫자를 상수 이름으로 바꾸면, <b>뜻이 분명해지고 고칠 때도 한 곳만 고치면</b> 됩니다.' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: PEP 8 이름 규칙 한 장 정리', html: '<p>파이썬 공식 스타일 가이드 <b>PEP 8</b> 의 이름 규칙입니다. 지키지 않아도 동작하지만, 이름만 보고 역할을 짐작할 수 있어 협업할 때 큰 도움이 됩니다.</p><ul><li><b>변수 · 함수</b>: 소문자 + 언더바 — <code>student_count</code>, <code>get_score()</code></li><li><b>상수</b>: 대문자 + 언더바 — <code>MAX_SPEED</code>, <code>PI</code></li><li><b>클래스</b>: 첫 글자 대문자 — <code>StudentRecord</code> (12장)</li><li><b>내부용 · 임시</b>: 언더바로 시작 — <code>_cache</code>, 값을 안 쓸 때는 <code>_</code></li><li>한 글자 이름 중 <code>l</code>(엘), <code>O</code>(오), <code>I</code>(아이)는 1 · 0 과 헷갈리므로 쓰지 않습니다.</li></ul><p>강의자료의 <code>myVar</code> 같은 camelCase 도 흔히 쓰이지만, 파이썬 생태계에서는 <code>my_var</code> 가 표준입니다. <b>한 프로젝트 안에서는 반드시 한 가지로 통일</b>하세요.</p>' }
        ],
        practice: [
          { title: '실습 3-11. 변수 값 추적하기', level: 1,
            ...
            solution: 'a = 10\nb = a + 5\na = a * 2\nc = a + b\nprint(a, b, c)\nb = b + c\nprint(a, b, c)\ntotal = a + b + c\nprint(total)\n',
            expect: '20 15 35\n20 50 35\n105' },
          { title: '실습 3-12. 변수 이름 고치기', level: 1,
            desc: '<p>아래 코드는 변수명 규칙을 어겨 실행되지 않습니다. <b>세 곳</b>을 고쳐 실행되게 만들고, 마지막에 세 값을 f-문자열로 출력하세요.</p><pre>2nd_score = 88      ← 숫자로 시작\nmy score = 90       ← 공백\nclass = "3학년 1반"  ← 예약어</pre><pre>3학년 1반 : 90점, 88점</pre>',
            hint: '숫자로 시작할 수 없으니 <code>score2</code>, 공백 대신 언더바 <code>my_score</code>, 예약어 대신 <code>class_name</code> 처럼 바꿉니다. PEP 8 은 소문자 + 언더바를 권합니다.',
            starter: '# 아래 세 줄의 변수명을 규칙에 맞게 고치세요\nscore2 = 88\nmy_score = 90\nclass_name = "3학년 1반"\n# TODO: f-문자열로 "3학년 1반 : 90점, 88점" 출력\n',
            solution: 'score2 = 88\nmy_score = 90\nclass_name = "3학년 1반"\nprint(f"{class_name} : {my_score}점, {score2}점")\n',
            expect: '3학년 1반 : 90점, 88점' },
          { title: '실습 3-13. 컵 바꾸기 (값 교환)', level: 2,
            ...
            solution: 'cup1 = "우유"\ncup2 = "주스"\ntemp = cup1\ncup1 = cup2\ncup2 = temp\n\nprint("cup1 :", cup1)\nprint("cup2 :", cup2)\n',
            expect: 'cup1 : 주스\ncup2 : 우유' },
          { title: '실습 3-14. 이름표 실험 (is 와 ==)', level: 2,
            desc: '<p>아래 코드를 실행하기 <b>전에</b> ①②③ 의 결과를 예측한 뒤, 빈 줄을 채워 확인하세요.</p><ol><li><code>b = a</code> 직후 두 이름은 같은 값을 가리키는가?</li><li>b 에 새 값을 대입하면 a 는 어떻게 되는가?</li><li>그 뒤 <code>a is b</code> 와 <code>a == b</code> 는 각각 무엇인가?</li></ol><pre>①  True True\n②  500 700\n③  False False</pre>',
            hint: '<code>is</code> 는 “같은 값(객체)인가”, <code>==</code> 는 “값이 같은가”입니다. 대입은 <b>이름표를 옮기는 것</b>이지 원래 값을 바꾸는 것이 아닙니다.',
            starter: 'a = 500\nb = a\nprint("① ", a is b, a == b)\n# TODO: b 에 700 을 대입하고 ② ③ 을 출력하세요\n',
            solution: 'a = 500\nb = a\nprint("① ", a is b, a == b)\nb = 700\nprint("② ", a, b)\nprint("③ ", a is b, a == b)\n',
            expect: '①  True True\n②  500 700\n③  False False' },
          { title: '🚀 프로젝트: 길이 단위 환산기', level: 3,
            desc: '<p>길이를 미터(m)로 입력받아 여러 단위로 환산해 <b>표 모양</b>으로 출력하는 프로그램을 만드세요. 이번 장에서 배운 <b>변수 · 상수 · f-문자열 서식</b>을 모두 쓰는 종합 과제입니다.</p><h4>요구 사항</h4><ul><li><b>입력</b>: <code>길이(m) 입력 : </code> 으로 실수 하나를 받습니다 (<code>float(input(...))</code>).</li><li><b>상수</b>: 환산 계수를 <b>대문자 이름의 상수</b>로 선언합니다 — <code>CM_PER_M = 100</code>, <code>KM_PER_M = 0.001</code>, <code>INCH_PER_M = 39.3701</code>, <code>FEET_PER_M = 3.28084</code></li><li><b>출력</b>: 단위 이름은 <b>왼쪽 8칸</b>, 값은 <b>오른쪽 20칸 · 천 단위 쉼표 · 소수점 아래 2자리</b>(km 만 4자리)로 출력합니다.</li><li>표의 위 · 아래와 제목 줄 아래에 <code>-</code> 28개로 구분선을 넣습니다. 구분선은 <code>"-" * 28</code> 로 만들어 <b>변수에 담아</b> 재사용하세요.</li></ul><h4>실행 예</h4><pre>길이(m) 입력 : 1234.5\n----------------------------\nunit                   value\n----------------------------\nm                   1,234.50\ncm                123,450.00\nkm                    1.2345\ninch               48,602.39\nfeet                4,050.20\n----------------------------</pre><h4>여기까지 했다면</h4><ul><li>야드(1 m = 1.09361 yd)와 마일(1 m = 0.000621371 mi) 줄을 추가해 보세요.</li><li>입력을 인치로 바꿔 미터를 구하는 <b>반대 방향</b> 프로그램도 만들어 보세요.</li><li>음수나 글자를 입력하면 어떻게 되는지 확인해 보세요. (해결 방법은 5교시에서 다룹니다)</li></ul>',
            hint: '값 줄은 <code>print(f"{\'cm\':&lt;8}{meter * CM_PER_M:&gt;20,.2f}")</code> 형태입니다. 구분선은 <code>line = "-" * 28</code> 로 만들어 두고 <code>print(line)</code> 으로 세 번 씁니다.',
            starter: 'CM_PER_M = 100\nKM_PER_M = 0.001\nINCH_PER_M = 39.3701\nFEET_PER_M = 3.28084\n\nmeter = float(input("길이(m) 입력 : "))\nline = "-" * 28\n\nprint(line)\nprint(f"{\'unit\':<8}{\'value\':>20}")\nprint(line)\nprint(f"{\'m\':<8}{meter:>20,.2f}")\n# TODO: cm, km, inch, feet 줄을 추가하고 마지막에 구분선을 출력하세요\n',
            solution: 'CM_PER_M = 100\nKM_PER_M = 0.001\nINCH_PER_M = 39.3701\nFEET_PER_M = 3.28084\n\nmeter = float(input("길이(m) 입력 : "))\nline = "-" * 28\n\nprint(line)\nprint(f"{\'unit\':<8}{\'value\':>20}")\nprint(line)\nprint(f"{\'m\':<8}{meter:>20,.2f}")\nprint(f"{\'cm\':<8}{meter * CM_PER_M:>20,.2f}")\nprint(f"{\'km\':<8}{meter * KM_PER_M:>20,.4f}")\nprint(f"{\'inch\':<8}{meter * INCH_PER_M:>20,.2f}")\nprint(f"{\'feet\':<8}{meter * FEET_PER_M:>20,.2f}")\nprint(line)\n',
            stdin: '1234.5\n',
            expect: '길이(m) 입력 : 1234.5\n----------------------------\nunit                   value\n----------------------------\nm                   1,234.50\ncm                123,450.00\nkm                    1.2345\ninch               48,602.39\nfeet                4,050.20\n----------------------------' }
        ],
        quiz: [
          ...
          { q: '<b>오류 찾기</b> — 첫 글자를 대문자로 바꾸려 한 코드입니다. 실행하면?<pre><code>s = "python"\ns[0] = "P"\nprint(s)</code></pre>', options: ['Python 이 출력된다', 'python 이 출력된다', 'TypeError — 문자열은 글자 하나를 바꿀 수 없다', 'IndexError — 0번 글자가 없다'], answer: 2,
            explain: '문자열은 <b>불변(immutable)</b>이라 일부만 바꿀 수 없습니다. <code>s = "P" + s[1:]</code> 처럼 <b>새 문자열을 만들어 다시 대입</b>해야 합니다.' }
        ],
        slides: [
          ...
          { layout: 'code', title: '한 걸음 더: 변수는 값에 붙인 이름표', code: 'a = 100\nb = a\nprint("처음:", a is b, id(a) == id(b))\n\nb = 200\nprint("대입 후:", a, b, a is b)\nprint("값 비교:", b == 200)',
            points: ['값은 메모리에, 변수는 그 값에 붙은 이름표', 'b = a → 같은 값에 이름표 하나 더', 'b = 200 → b 이름표만 옮겨 감 (a 는 그대로)', 'is = 같은 객체인가 / == = 값이 같은가'],
            notes: '<p>“그릇” 비유로 충분히 이해한 뒤 보여 주는 <b>한 단계 깊은 모델</b>입니다. 리스트(8장)에서 이 차이가 결정적으로 중요해진다고 예고하세요.</p><p>이름표가 하나도 없어진 값은 파이썬이 자동으로 정리합니다(가비지 컬렉션). C 언어처럼 직접 메모리를 반납하지 않아도 됩니다.</p><p>시간: 5분</p>' },
          { layout: 'two', title: '문자열은 바꿀 수 없다 (불변)',
            left: { title: '새 문자열이 만들어진다', code: 's = "python"\nt = s\ns = s + "!"\nprint(s, t)\nprint(t.upper(), t)' },
            right: { title: '일부만 바꾸면 TypeError', code: 's = "python"\ntry:\n    s[0] = "P"\nexcept TypeError as e:\n    print("오류:", e)\ns = "P" + s[1:]\nprint(s)' },
            notes: '<p>왼쪽: <code>upper()</code> 는 원본을 바꾸지 않고 <b>새 문자열을 돌려준다</b> — 초보자가 가장 많이 하는 실수(<code>t.upper()</code> 만 쓰고 결과를 버리는 것)입니다.</p><p>오른쪽의 try/except 는 13장 내용이니 “오류를 붙잡아 메시지만 보여 주는 장치” 정도로만 설명합니다.</p><p>시간: 4분</p>' },
          ...
          { layout: 'practice', title: '실습 3-11. 변수 값 추적하기', desc: '실행 전에 각 print() 결과를 예측하고, total = a + b + c 를 추가해 출력하세요.',
            starter: 'a = 10\nb = a + 5\na = a * 2\nc = a + b\nprint(a, b, c)\nb = b + c\nprint(a, b, c)\n# TODO: total 에 a + b + c 를 넣고 출력\n',
            solution: 'a = 10\nb = a + 5\na = a * 2\nc = a + b\nprint(a, b, c)\nb = b + c\nprint(a, b, c)\ntotal = a + b + c\nprint(total)\n',
            notes: '<p>칠판에 a, b, c 열의 표를 그려 한 줄씩 값을 채워 가며 함께 추적합니다.</p><p>시간: 6분</p>' },
          { layout: 'summary', title: '정리', bullets: ['변수 = 값을 담는 이름 붙은 그릇 (파이썬은 선언 없이 대입으로 생성)', 'type() 으로 bool · int · float · str 확인', '규칙: 대소문자 구분, 숫자 시작 ✘, 예약어 ✘', '= 는 대입: 오른쪽 계산 → 왼쪽 변수에', '연속 대입, 자기 자신에게 다시 대입, 데이터형은 바뀔 수 있다', '(중급) 변수 = 값에 붙인 이름표 · 문자열은 불변 · 상수는 대문자 이름'],
      ...
      /* ───────────────────────── 4교시: 비트 · 바이트 · 진수 ───────────────────────── */
      {
        ...
        goals: [
          ...
          'f-문자열의 진수 서식(b · o · x · X · # · 08b)으로 자릿수를 맞춰 출력할 수 있다',
        ...
        content: [
          ...
          { type: 'h', text: '한 걸음 더: 긴 숫자를 읽기 쉽게 쓰기' },
          { type: 'p', html: '<code>51700000</code> 처럼 0 이 많은 숫자는 자릿수를 세기 어렵습니다. 파이썬은 숫자 중간에 <b>밑줄(_)</b>을 넣어 읽기 좋게 쓸 수 있게 해 줍니다. 밑줄은 <b>사람 눈에만 보이는 구분선</b>이라 값에는 아무 영향이 없습니다. 2진수는 4자리씩, 16진수는 바이트 단위로 끊어 쓰면 훨씬 잘 읽힙니다.' },
          { type: 'code', title: '추가 예제. 숫자 안의 밑줄과 진수 서식', code: 'population = 51_700_000\nmask = 0b1010_1010\ncolor = 0x_FF_88_00\nprint(population, mask, color)\nprint(f"{population:,}")\n\nnum = 147\nprint(f"{num:b} {num:o} {num:x} {num:X}")\nprint(f"{num:#b} {num:#o} {num:#x}")\nprint(f"{num:08b} | {num:#010b} | {num:_b}")\nprint(num.bit_length(), "비트면 충분")',
            expect: '51700000 170 16746496\n51,700,000\n10010011 223 93 93\n0b10010011 0o223 0x93\n10010011 | 0b10010011 | 1001_0011\n8 비트면 충분',
            desc: '<code>9행</code> 서식에 <code>#</code> 을 붙이면 <b>접두어까지</b> 붙습니다. <code>10행</code> <code>08b</code> 는 8자리에 0 채움, <code>#010b</code> 는 접두어를 포함해 10칸, <code>_b</code> 는 4자리마다 밑줄입니다. <code>11행</code> <code>bit_length()</code> 는 그 수를 나타내는 데 <b>비트가 몇 개 필요한지</b> 알려 줍니다(147 → 8비트 = 1바이트).' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: 음수의 2진수와 2의 보수', html: '<p><code>bin(-5)</code> 는 <code>\'-0b101\'</code> 처럼 <b>부호를 앞에 붙여</b> 보여 줍니다. 사람이 읽기엔 편하지만, 실제 컴퓨터가 메모리에 저장하는 모양은 아닙니다.</p><p>컴퓨터는 음수를 <b>2의 보수(two\'s complement)</b> 로 저장합니다. 1바이트라면 −5 는 <code>11111011</code> 입니다. 맨 앞 비트가 1 이면 음수라는 뜻이고, 이렇게 하면 <b>뺄셈을 덧셈 회로로 처리</b>할 수 있어 회로가 단순해집니다. 파이썬에서는 <code>-5 &amp; 0xFF</code> 처럼 1바이트만 남기면 저장 모양을 볼 수 있습니다.</p>' },
          { type: 'code', title: '추가 예제. 음수의 2진수', code: 'n = -5\nprint(bin(n), hex(n))\nprint(f"1바이트 저장 모양: {n & 0xFF:08b}")\nprint("필요한 비트 수:", n.bit_length())',
            expect: '-0b101 -0x5\n1바이트 저장 모양: 11111011\n필요한 비트 수: 3',
            desc: '<code>&amp;</code> 는 비트 단위 연산자입니다(4장). 지금은 “아래 8비트만 잘라 본다”는 뜻으로만 이해하면 충분합니다.' },
          { type: 'h', text: '진수는 어디에 쓰일까: 글자에도 번호가 있다' },
          { type: 'p', html: '컴퓨터는 글자도 숫자로 저장합니다. 글자에 매겨진 번호를 <b>문자 코드</b>라고 하고, <code>ord()</code> 로 번호를, <code>chr()</code> 로 번호에 해당하는 글자를 얻습니다. 유니코드 문서에서는 이 번호를 <b>항상 16진수</b>로 적습니다(예: U+AC00 = 가).' },
          { type: 'code', title: '추가 예제. ord() 와 chr() 로 문자 코드 보기', code: 'print(ord("A"), hex(ord("A")))\nprint(ord("a"), hex(ord("a")))\nprint(ord("가"), hex(ord("가")))\nprint(chr(0x41), chr(0x61), chr(0xAC00))\nprint("\\u0041 \\uac00")',
            expect: '65 0x41\n97 0x61\n44032 0xac00\nA a 가\nA 가',
            desc: '<code>5행</code> 2교시에 배운 <code>\\u</code> 이스케이프가 바로 이 16진수 번호입니다. 대문자 A(65)와 소문자 a(97)가 32 차이라는 점도 확인해 보세요 — 2진수로 보면 딱 한 비트 차이입니다(<code>1000001</code> / <code>1100001</code>).' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: 16진수를 만나는 곳들', html: '<ul><li><b>웹 색상</b> <code>#FF8800</code> — 빨강 · 초록 · 파랑 각 1바이트</li><li><b>유니코드</b> U+AC00 — 글자 번호</li><li><b>메모리 주소</b> <code>0x7ffd3c2a</code> — 오류 메시지에서 자주 보입니다</li><li><b>파일 권한</b> <code>0o755</code> — 리눅스에서는 8진수(3비트씩 읽기 · 쓰기 · 실행)</li><li><b>MAC 주소 · 해시 값</b> — 바이트를 두 글자씩 표시</li></ul><p>공통점은 <b>1바이트를 정확히 두 글자로 적을 수 있다</b>는 것입니다. 이것이 16진수가 컴퓨터 세계의 공용어인 이유입니다.</p>' }
        ],
        practice: [
          ...
          { title: '실습 3-16. 10진수를 여러 진수로', level: 1,
            ...
            stdin: '200\n',
            expect: '10진수 입력 : 200\n2진수 : 0b11001000\n8진수 : 0o310\n16진수 : 0xc8' },
          { title: '실습 3-17. 접두어 없이 자릿수 맞춰 출력', level: 1,
            desc: '<p><code>bin()</code> · <code>hex()</code> 는 <code>0b</code> · <code>0x</code> 접두어가 붙습니다. 이번에는 <b>f-문자열 서식</b>으로 접두어 없이, 자릿수를 맞춰 출력하세요.</p><ul><li>13 을 <b>8자리 2진수</b>(0 채움)</li><li>13 을 <b>2자리 16진수 소문자</b>(0 채움)</li><li>13 을 <b>접두어 포함 6칸 16진수</b></li><li>255 를 <b>8자리 2진수</b></li></ul><pre>00001101\n0d\n0x000d\n11111111</pre>',
            hint: '<code>f"{num:08b}"</code>, <code>f"{num:02x}"</code>, <code>f"{num:#06x}"</code> 형식입니다. <code>#</code> 은 접두어를 붙이라는 뜻입니다.',
            starter: 'num = 13\nprint(f"{num:08b}")\n# TODO: 2자리 16진수 소문자\n# TODO: 접두어 포함 6칸 16진수\n# TODO: 255 를 8자리 2진수로\n',
            solution: 'num = 13\nprint(f"{num:08b}")\nprint(f"{num:02x}")\nprint(f"{num:#06x}")\nprint(f"{255:08b}")\n',
            expect: '00001101\n0d\n0x000d\n11111111' },
          { title: '실습 3-18. 웹 색상 코드 해석하기', level: 2,
            ...
            expect: '빨강 : 255\n초록 : 136\n파랑 : 0' },
          { title: '실습 3-19. 반대로: RGB 값으로 색상 코드 만들기', level: 2,
            desc: '<p>이번에는 10진수 밝기 세 개(0~255)를 받아 <b>웹 색상 코드</b>를 만들어 출력하세요. 각 값은 <b>두 자리 16진수</b>여야 하므로 0 을 채웁니다. 대문자 코드와 소문자 코드를 모두 출력하고, 마지막 줄에는 세 값을 <b>8자리 2진수</b>로 출력합니다.</p><pre>#FF8800\n#ff8800\n11111111 10001000 00000000</pre>',
            hint: '<code>f"#{red:02X}{green:02X}{blue:02X}"</code> — <code>02X</code> 는 “두 자리, 0 채움, 대문자 16진수”입니다.',
            starter: 'red = 255\ngreen = 136\nblue = 0\nprint(f"#{red:02X}{green:02X}{blue:02X}")\n# TODO: 소문자 코드\n# TODO: 세 값을 8자리 2진수로 (빈칸으로 구분)\n',
            solution: 'red = 255\ngreen = 136\nblue = 0\nprint(f"#{red:02X}{green:02X}{blue:02X}")\nprint(f"#{red:02x}{green:02x}{blue:02x}")\nprint(f"{red:08b} {green:08b} {blue:08b}")\n',
            expect: '#FF8800\n#ff8800\n11111111 10001000 00000000' },
          { title: '🚀 프로젝트: 진법 변환 학습 도구', level: 3,
            desc: '<p>10진수를 입력하면 <b>네 가지 진수로 한눈에 보여 주고 검산까지 해 주는</b> 학습 도구를 만드세요. 진수 개념과 f-문자열 서식을 모두 쓰는 종합 과제입니다.</p><h4>요구 사항</h4><ul><li><b>입력</b>: <code>10진수 입력 : </code> 으로 정수 하나 (<code>int(input(...))</code>)</li><li><b>표</b>: 2 · 8 · 10 · 16진수를 <b>접두어 있는 표기</b>(<code>bin()</code> · <code>oct()</code> · <code>hex()</code>)와 <b>접두어 없는 표기</b>(서식 <code>b</code> · <code>o</code> · <code>X</code>) 두 열로 출력</li><li>열 폭은 <code>base</code> 왼쪽 6칸, 접두어 열 오른쪽 16칸, 평문 열 오른쪽 12칸</li><li>구분선 <code>"-" * 34</code> 를 변수에 담아 표의 위 · 제목 아래 · 표 끝에 사용</li><li><b>추가 정보</b>: 8자리 2진수(0 채움), <code>bit_length()</code> 로 필요한 비트 수</li><li><b>검산</b>: 16진수 표기를 <code>int(문자열, 16)</code> 으로 되돌려 원래 값이 나오는지 출력</li></ul><h4>실행 예</h4><pre>10진수 입력 : 147\n----------------------------------\nbase       with prefix       plain\n----------------------------------\n2           0b10010011    10010011\n8                0o223         223\n10                 147         147\n16                0x93          93\n----------------------------------\n8비트 2진수 : 10010011\n필요한 비트 수 : 8\n검산 : int(\'93\', 16) = 147</pre><h4>여기까지 했다면</h4><ul><li>입력 진수를 고르게 해 보세요([프로그램 2]처럼 <code>if</code> 사용).</li><li>글자를 입력했을 때 오류 대신 안내 메시지가 나오게 해 보세요(<code>try / except ValueError</code>, 5교시 맛보기).</li><li>16비트 · 32비트 2진수도 함께 보여 주고, 음수를 넣으면 어떻게 되는지 실험해 보세요.</li></ul>',
            hint: '표의 한 줄은 <code>print(f"{\'2\':&lt;6}{bin(num):&gt;16}{num:&gt;12b}")</code> 형태입니다. 같은 <code>num</code> 을 서식만 바꿔 여러 번 쓰는 것이 핵심입니다.',
            starter: 'num = int(input("10진수 입력 : "))\nline = "-" * 34\n\nprint(line)\nprint(f"{\'base\':<6}{\'with prefix\':>16}{\'plain\':>12}")\nprint(line)\nprint(f"{\'2\':<6}{bin(num):>16}{num:>12b}")\n# TODO: 8진수, 10진수, 16진수 줄을 추가하세요\n# TODO: 구분선, 8비트 2진수, 비트 수, 검산 줄을 추가하세요\n',
            solution: 'num = int(input("10진수 입력 : "))\nline = "-" * 34\n\nprint(line)\nprint(f"{\'base\':<6}{\'with prefix\':>16}{\'plain\':>12}")\nprint(line)\nprint(f"{\'2\':<6}{bin(num):>16}{num:>12b}")\nprint(f"{\'8\':<6}{oct(num):>16}{num:>12o}")\nprint(f"{\'10\':<6}{num:>16}{num:>12}")\nprint(f"{\'16\':<6}{hex(num):>16}{num:>12X}")\nprint(line)\nprint(f"8비트 2진수 : {num:08b}")\nprint(f"필요한 비트 수 : {num.bit_length()}")\nprint(f"검산 : int(\'{num:X}\', 16) = {int(format(num, \'X\'), 16)}")\n',
            stdin: '147\n',
            expect: '10진수 입력 : 147\n----------------------------------\nbase       with prefix       plain\n----------------------------------\n2           0b10010011    10010011\n8                0o223         223\n10                 147         147\n16                0x93          93\n----------------------------------\n8비트 2진수 : 10010011\n필요한 비트 수 : 8\n검산 : int(\'93\', 16) = 147' }
        ],
        quiz: [
          ...
          { q: '<b>오류 찾기</b> — 2진수로 읽으려고 쓴 코드입니다. 실행하면?<pre><code>print(int("123", 2))</code></pre>', options: ['123', '27', '7', 'ValueError — 2진수에는 2 와 3 이 없다'], answer: 3,
            explain: '<code>int(문자열, 2)</code> 는 0 과 1 만 받아들입니다. <code>invalid literal for int() with base 2</code> 오류가 납니다. 진수와 값이 맞는지 항상 확인하세요.' }
        ],
        slides: [
          ...
          { layout: 'code', title: '한 걸음 더: 밑줄과 진수 서식', code: 'population = 51_700_000\nmask = 0b1010_1010\nprint(population, mask)\n\nnum = 147\nprint(f"{num:b} {num:o} {num:x} {num:X}")\nprint(f"{num:#b} {num:#x}")\nprint(f"{num:08b} | {num:_b}")\nprint(num.bit_length(), "비트면 충분")',
            points: ['숫자 안의 _ 는 사람 눈에만 보이는 구분선', '서식 b · o · x · X → 접두어 없음', '# 을 붙이면 0b · 0x 접두어까지', '08b 0 채움, _b 4자리마다 밑줄', 'bit_length() → 필요한 비트 수'],
            notes: '<p>bin()/hex() 는 접두어가 붙어 <b>문자열</b>을 돌려주고, 서식은 접두어 없이 <b>자릿수까지</b> 맞출 수 있다는 차이를 강조합니다.</p><p><b>발문</b>: “147 을 저장하려면 몇 비트가 필요할까요?” → 8비트(1바이트).</p><p>시간: 5분</p>' },
          { layout: 'code', title: '글자에도 번호가 있다 (ord · chr)', code: 'print(ord("A"), hex(ord("A")))\nprint(ord("가"), hex(ord("가")))\nprint(chr(0x41), chr(0xAC00))\nprint("\\u0041 \\uac00")\n\nn = -5\nprint(bin(n), f"{n & 0xFF:08b}")',
            points: ['ord() 글자 → 번호, chr() 번호 → 글자', '유니코드 번호는 늘 16진수 (U+AC00)', '2교시의 \\u 이스케이프가 바로 이 번호', '음수는 실제로 2의 보수로 저장된다'],
            notes: '<p>진수가 “시험용 계산”이 아니라 <b>실제로 쓰이는 표기법</b>임을 보여 주는 슬라이드입니다. 웹 색상 · 유니코드 · 파일 권한(0o755) · 메모리 주소를 예로 듭니다.</p><p>A(65)와 a(97)의 차이 32 가 2진수에서 한 비트 차이라는 것도 보여 주면 좋습니다.</p><p>시간: 4분</p>' },
          ...
          { layout: 'practice', title: '🚀 프로젝트: 진법 변환 학습 도구', desc: '10진수를 입력하면 2 · 8 · 10 · 16진수를 접두어 있는 표기와 없는 표기 두 열의 표로 보여 주고, 8비트 2진수 · 필요한 비트 수 · 검산까지 출력하세요.',
            starter: 'num = int(input("10진수 입력 : "))\nline = "-" * 34\n\nprint(line)\nprint(f"{\'base\':<6}{\'with prefix\':>16}{\'plain\':>12}")\nprint(line)\nprint(f"{\'2\':<6}{bin(num):>16}{num:>12b}")\n# TODO: 8 · 10 · 16진수 줄과 검산 줄을 추가하세요\n',
            solution: 'num = int(input("10진수 입력 : "))\nline = "-" * 34\n\nprint(line)\nprint(f"{\'base\':<6}{\'with prefix\':>16}{\'plain\':>12}")\nprint(line)\nprint(f"{\'2\':<6}{bin(num):>16}{num:>12b}")\nprint(f"{\'8\':<6}{oct(num):>16}{num:>12o}")\nprint(f"{\'10\':<6}{num:>16}{num:>12}")\nprint(f"{\'16\':<6}{hex(num):>16}{num:>12X}")\nprint(line)\nprint(f"8비트 2진수 : {num:08b}")\nprint(f"검산 : int(\'{num:X}\', 16) = {int(format(num, \'X\'), 16)}")\n',
            stdin: '147\n',
            notes: '<p>남은 시간이 적으면 표의 두 줄만 완성해도 성공으로 인정합니다. 147 · 255 · 1024 를 차례로 넣어 보며 자릿수 변화를 관찰하게 하세요.</p><p>빨리 끝낸 학생에게는 확장 과제(입력 진수 선택, try/except, 16비트 출력)를 안내합니다.</p><p>시간: 10분</p>' },
      ...
      /* ───────────────────────── 5교시: 기본 데이터형 ───────────────────────── */
      {
        ...
        goals: [
          ...
          '실수 오차의 원인을 설명하고 round · math.isclose · decimal 로 안전하게 비교 · 계산할 수 있다',
          'type() 과 isinstance() 의 차이, None 과 참 · 거짓으로 취급되는 값을 설명할 수 있다',
        ...
        content: [
          ...
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: 파이썬 정수는 왜 한계가 없을까 (임의 정밀도)', html: '<p>C 나 자바의 <code>int</code> 는 <b>32비트 · 64비트로 크기가 고정</b>되어 있습니다. 그래서 최대값을 넘으면 값이 갑자기 음수로 바뀌는 <b>오버플로(overflow)</b> 가 일어납니다. 은행 · 게임에서 점수가 음수가 되는 유명한 버그가 이것입니다.</p><p>파이썬의 정수는 <b>임의 정밀도(arbitrary precision)</b> 라서, 값이 커지면 메모리를 더 써서 자릿수를 늘립니다. 한계는 오직 <b>컴퓨터의 메모리</b>뿐입니다. 대신 아주 큰 수의 계산은 고정 크기 정수보다 느립니다. “정확함을 얻고 속도를 조금 내주었다”고 이해하면 됩니다.</p>' },
          { type: 'code', title: '추가 예제. 값이 커지면 메모리도 커진다 (sys.getsizeof)', code: 'import sys\n\nsmall = sys.getsizeof(1)\nbig = sys.getsizeof(10 ** 100)\nprint("작은 정수 :", small, "바이트")\nprint("100자리 정수 :", big, "바이트")\nprint("커질수록 메모리도 커진다 :", big > small)\n\n# 64비트 정수의 한계를 넘어도 파이썬은 정확하다\nprint(2 ** 63 - 1)\nprint(2 ** 63)\nprint((2 ** 63) * (2 ** 63))',
            expect: '작은 정수 : 28 바이트\n100자리 정수 : 72 바이트\n커질수록 메모리도 커진다 : True\n9223372036854775807\n9223372036854775808\n85070591730234615865843651857942052864',
            desc: '<code>sys.getsizeof()</code> 는 값 하나가 차지하는 메모리 크기(바이트)를 알려 줍니다. 정수 1 하나에도 수십 바이트가 드는 것은 파이썬의 모든 값이 <b>객체</b>라서 부가 정보를 함께 담기 때문입니다. <code>10~12행</code> C 언어라면 넘칠 값도 파이썬은 정확하게 계산합니다.' },
          ...
          { type: 'h', text: '한 걸음 더: 실수를 안전하게 비교하고 계산하기' },
          { type: 'p', html: '실수의 오차는 “신기한 현상”이 아니라 <b>실제로 프로그램을 망가뜨리는 문제</b>입니다. <code>0.1 + 0.2 == 0.3</code> 이 <code>False</code> 라서 결제 금액이 맞지 않거나, 반복이 끝나지 않는 일이 생깁니다. 원인은 간단합니다. <b>10진수 0.1 을 2진수로는 정확히 쓸 수 없기</b> 때문입니다. 10진수로 1/3 을 0.3333… 으로 영원히 적어야 하는 것과 같습니다.' },
          { type: 'code', title: '추가 예제. 0.1 의 진짜 모습과 안전한 비교', code: 'import math\n\nprint(0.1 + 0.2)\nprint(0.1 + 0.2 == 0.3)\nprint(format(0.1, ".20f"))\n\n# 방법 1: 자릿수를 정해 반올림한 뒤 비교\nprint(round(0.1 + 0.2, 10) == round(0.3, 10))\n\n# 방법 2: "거의 같은가" 를 묻는다 (권장)\nprint(math.isclose(0.1 + 0.2, 0.3))',
            expect: '0.30000000000000004\nFalse\n0.10000000000000000555\nTrue\nTrue',
            desc: '<code>5행</code> 0.1 을 소수점 아래 20자리까지 보면 실제로는 0.1 보다 아주 조금 큽니다. <code>8행</code> 원하는 자릿수로 반올림해 비교하거나, <code>11행</code> <code>math.isclose()</code> 로 <b>“거의 같은가”</b>를 묻는 것이 안전합니다. 실수끼리 <code>==</code> 로 비교하는 코드를 보면 일단 의심하세요.' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: 돈 계산에는 decimal', html: '<p><code>decimal</code> 모듈의 <code>Decimal</code> 은 <b>10진수를 그대로</b> 저장해 오차가 없습니다. 금액 · 세금 · 이자처럼 <b>1원도 틀리면 안 되는 계산</b>에 씁니다. 대신 float 보다 느리므로, 과학 계산이나 그래픽처럼 속도가 중요한 곳에는 float 을 씁니다.</p><p>주의: <code>Decimal(0.1)</code> 처럼 <b>실수를 넣으면 이미 오차가 섞인 값</b>이 들어갑니다. 반드시 <code>Decimal("0.1")</code> 처럼 <b>문자열</b>로 넘기세요.</p>' },
          { type: 'code', title: '추가 예제. decimal 로 정확한 금액 계산', code: 'from decimal import Decimal\n\nprint(0.1 + 0.2 == 0.3)\nprint(Decimal("0.1") + Decimal("0.2") == Decimal("0.3"))\n\nprice = Decimal("19900")\nrate = Decimal("0.1")\nprint("부가세 :", price * rate)\nprint("합계 :", price + price * rate)\n\nprint("실수로 만들면 오차가 섞인다:", Decimal(0.1))',
            expect: 'False\nTrue\n부가세 : 1990.0\n합계 : 21890.0\n실수로 만들면 오차가 섞인다: 0.1000000000000000055511151231257827021181583404541015625',
            desc: '<code>10행</code>이 오차 이야기의 결론입니다. 실수 <code>0.1</code> 을 그대로 넣으면 <b>컴퓨터가 실제로 저장한 값</b>이 그대로 드러납니다. 문자열로 넘겨야 우리가 뜻한 0.1 이 됩니다.' },
          ...
          { type: 'h', text: '한 걸음 더: type() 과 isinstance()' },
          { type: 'p', html: '데이터형을 확인하는 방법은 두 가지입니다. <code>type(x) == int</code> 는 <b>“정확히 그 형인가”</b>를 묻고, <code>isinstance(x, int)</code> 는 <b>“그 형(또는 그 자식 형)인가”</b>를 묻습니다. 실무에서는 대부분 <code>isinstance()</code> 를 씁니다. <b>여러 형을 한 번에</b> 물어볼 수 있고(<code>isinstance(x, (int, float))</code>), 형을 물려받은 경우까지 포함하기 때문입니다.' },
          { type: 'code', title: '추가 예제. type() 과 isinstance() 의 차이', code: 'print(type(10) == int, isinstance(10, int))\nprint(isinstance(3.14, (int, float)))\nprint(isinstance("10", (int, float)))\n\n# bool 은 int 를 물려받은 형이다\nprint(type(True) == int, isinstance(True, int))\nprint(True + True, True * 10, False + 5)',
            expect: 'True True\nTrue\nFalse\nFalse True\n2 10 5',
            desc: '<code>6~7행</code> 놀랍게도 파이썬의 <b><code>True</code> 는 1, <code>False</code> 는 0 으로 계산</b>됩니다. <code>bool</code> 이 <code>int</code> 의 자식 형이기 때문입니다. 이 성질은 “조건에 맞는 개수 세기”에 요긴하게 쓰입니다.' },

          { type: 'h', text: '한 걸음 더: None 과 진리값' },
          { type: 'p', html: '<b><code>None</code></b> 은 “값이 없음”을 나타내는 특별한 값입니다. 0 이나 빈 문자열과는 다릅니다 — 0 은 “숫자 영”이라는 값이지만 None 은 <b>아직 아무것도 정해지지 않았다</b>는 뜻입니다. “값이 준비되었는지” 표시하는 변수의 첫 값으로 자주 씁니다.' },
          { type: 'p', html: '또 파이썬은 불형이 아닌 값도 참 · 거짓으로 판단합니다. <b>비어 있거나 0 인 값은 거짓(falsy)</b>, 나머지는 모두 참(truthy)입니다. 이 규칙 덕분에 <code>if name:</code> 처럼 짧고 읽기 좋은 조건을 쓸 수 있습니다(5장).' },
          { type: 'code', title: '추가 예제. None 과 참 · 거짓으로 취급되는 값', code: 'result = None\nprint(result, type(result), result is None)\n\nprint(bool(0), bool(0.0), bool(""), bool([]), bool(None))\nprint(bool(1), bool(-3), bool("0"), bool(" "), bool("False"))',
            expect: "None <class 'NoneType'> True\nFalse False False False False\nTrue True True True True",
            desc: '<code>4행</code> 0, 0.0, 빈 문자열, 빈 리스트, None 은 모두 <b>거짓</b>입니다. <code>5행</code>이 함정입니다 — 문자열 <code>"0"</code> 과 <code>"False"</code> 는 <b>글자가 들어 있으므로 참</b>입니다! 입력값을 그대로 조건에 쓰면 안 되는 이유입니다.' },
          { type: 'callout', kind: 'more', title: '📘 더 알아보기: None 은 == 이 아니라 is 로 비교', html: '<p>None 은 프로그램 전체에 <b>딱 하나만 존재하는 값</b>입니다. 그래서 <code>x == None</code> 이 아니라 <code>x is None</code> 으로 비교하는 것이 파이썬의 관례이며 PEP 8 의 권장 사항입니다. 더 빠르고, 값 비교 방식을 바꿔 놓은 특별한 객체에도 속지 않습니다.</p><p>함수가 아무것도 <code>return</code> 하지 않으면 자동으로 None 을 돌려줍니다(9장). <code>print()</code> 의 결과를 변수에 담아 보면 None 이 나오는 것도 같은 이유입니다.</p>' },
...
print(a)`, expect: '파이썬\n만세' },
          ...
          { type: 'h', text: '한 걸음 더: 입력값이 숫자가 아닐 때' },
          { type: 'p', html: '<code>int(input())</code> 은 편리하지만 <b>사용자가 숫자가 아닌 것을 입력하면 프로그램이 오류로 멈춥니다</b>. 실제 프로그램은 이런 상황을 미리 생각해 두어야 합니다. 오류를 붙잡아 안내 메시지를 보여 주는 장치가 <code>try / except</code> 입니다(13장에서 자세히 배웁니다).' },
          { type: 'code', title: '추가 예제. int() 가 실패하면 (ValueError)', code: 'num = int("열")\nprint(num)', expectError: true,
            expect: 'Traceback (most recent call last):\n  File "main.py", line 1, in <module>\n    num = int("열")\n          ~~~^^^^^^\nValueError: invalid literal for int() with base 10: \'열\'',
            desc: '<code>invalid literal</code> = “정수로 읽을 수 없는 글자”입니다. 앞 교시의 <code>int("123", 2)</code> 오류와 같은 종류입니다. <b>공백이 섞인 <code>" 12 "</code> 는 괜찮지만</b>, <code>"12.5"</code> 는 정수가 아니므로 실패합니다(<code>float("12.5")</code> 는 성공).' },
          { type: 'code', title: '추가 예제. try / except 로 안내 메시지 보여 주기 (맛보기)', code: 'text = input("나이 : ")\n\ntry:\n    age = int(text)\n    print(f"내년에는 {age + 1}살이 됩니다.")\nexcept ValueError:\n    print(f"숫자만 입력하세요. 입력하신 값: {text!r}")',
            stdin: '열일곱\n',
            expect: "나이 : 열일곱\n숫자만 입력하세요. 입력하신 값: '열일곱'",
            desc: '<code>try:</code> 아래 코드를 실행하다가 <code>ValueError</code> 가 나면 <code>except:</code> 아래로 건너뜁니다. 프로그램이 멈추지 않고 <b>사용자에게 무엇이 잘못됐는지</b> 알려 줍니다. <code>{text!r}</code> 로 따옴표까지 보여 주면 공백이 섞였는지도 확인할 수 있습니다. 예시 입력을 <code>17</code> 로 바꿔 다시 실행해 보세요.' },
        ...
        practice: [
          { title: '실습 3-21. 두 수의 연산 결과', level: 1,
            ...
            stdin: '9\n2\n',
            expect: '첫 번째 수 : 9\n두 번째 수 : 2\n9 + 2 = 11\n9 - 2 = 7\n9 * 2 = 18\n9 / 2 = 4.5\n9 // 2 = 4\n9 % 2 = 1\n9 ** 2 = 81' },
          { title: '실습 3-22. 초를 분과 초로', level: 2,
            ...
            stdin: '135\n',
            expect: '초 입력 : 135\n2분 15초\n2:15' },
          { title: '실습 3-23. 데이터형 맞히기', level: 1,
            ...
            expect: "<class 'int'>\n<class 'float'>\n<class 'str'>\n<class 'bool'>\n<class 'float'>" },
          { title: '실습 3-24. 실수 비교 바로잡기', level: 2,
            desc: '<p><code>0.1 + 0.2 == 0.3</code> 이 <code>False</code> 인 것을 직접 확인한 뒤, <b>두 가지 안전한 비교 방법</b>으로 True 가 나오게 고치세요. 마지막 줄에는 결과를 소수점 아래 두 자리로 보기 좋게 출력합니다.</p><pre>0.30000000000000004\n== 비교 : False\nround 비교 : True\nisclose 비교 : True\n보기 좋게 : 0.30</pre>',
            hint: '<code>round(a, 10) == round(b, 10)</code> 처럼 자릿수를 맞춰 비교하거나, <code>import math</code> 후 <code>math.isclose(a, b)</code> 를 씁니다.',
            starter: 'import math\n\na = 0.1 + 0.2\nb = 0.3\nprint(a)\nprint("== 비교 :", a == b)\n# TODO: round 를 이용한 비교\n# TODO: math.isclose 를 이용한 비교\n# TODO: 소수점 아래 두 자리로 출력\n',
            solution: 'import math\n\na = 0.1 + 0.2\nb = 0.3\nprint(a)\nprint("== 비교 :", a == b)\nprint("round 비교 :", round(a, 10) == round(b, 10))\nprint("isclose 비교 :", math.isclose(a, b))\nprint(f"보기 좋게 : {a:.2f}")\n',
            expect: '0.30000000000000004\n== 비교 : False\nround 비교 : True\nisclose 비교 : True\n보기 좋게 : 0.30' },
          { title: '🚀 프로젝트: 성적표 서식 출력기', level: 3,
            desc: '<p>이름과 세 과목 점수를 입력받아 <b>표 모양 성적표</b>를 출력하는 프로그램을 만드세요. 3장에서 배운 <b>입력 · 형 변환 · 변수 · 상수 · 연산자 · f-문자열 서식</b>을 모두 사용합니다.</p><h4>요구 사항</h4><ul><li><b>입력</b>: 이름(문자열), 국어 · 영어 · 수학 점수(정수). 점수는 <code>int(input(...))</code> 으로 변환합니다.</li><li><b>계산</b>: 총점(<code>total</code>)과 평균(<code>average</code>, 실수). 평균은 <b>소수점 아래 두 자리</b>로 출력합니다.</li><li><b>상수</b>: 만점을 <code>MAX_SCORE = 100</code> 상수로 선언하고, 각 과목의 달성률을 <b>백분율 서식(<code>:.1%</code>)</b>으로 출력합니다.</li><li><b>서식</b>: 과목 이름 왼쪽 10칸, 점수 오른쪽 8칸, 달성률 오른쪽 12칸. 표 위아래는 <code>=</code> 30개, 표 안쪽 구분선은 <code>-</code> 30개.</li></ul><h4>실행 예</h4><pre>이름 : 홍길동\n국어 : 92\n영어 : 87\n수학 : 78\n==============================\n홍길동 님의 성적표\n==============================\nsubject      score       ratio\n------------------------------\nkorean          92       92.0%\nenglish         87       87.0%\nmath            78       78.0%\n------------------------------\ntotal          257\naverage      85.67       85.7%\n==============================</pre><h4>여기까지 했다면</h4><ul><li>평균 점수를 <code>"#" * (점수 // 10)</code> 같은 막대로 그려 보세요.</li><li>과목을 다섯 개로 늘려 보고, 같은 코드가 반복되는 것이 불편하다면 — 그 해답이 7장의 반복문과 8장의 리스트입니다.</li><li>점수에 글자를 입력해도 멈추지 않도록 <code>try / except ValueError</code> 를 붙여 보세요.</li></ul>',
            hint: '한 줄은 <code>print(f"{\'korean\':&lt;10}{kor:&gt;8}{kor / MAX_SCORE:&gt;12.1%}")</code> 형태입니다. 구분선은 <code>line = "-" * 30</code> 으로 만들어 재사용하세요.',
            starter: 'MAX_SCORE = 100\n\nname = input("이름 : ")\nkor = int(input("국어 : "))\neng = int(input("영어 : "))\nmat = int(input("수학 : "))\n\nline = "-" * 30\nprint("=" * 30)\nprint(f"{name} 님의 성적표")\nprint("=" * 30)\nprint(f"{\'subject\':<10}{\'score\':>8}{\'ratio\':>12}")\nprint(line)\nprint(f"{\'korean\':<10}{kor:>8}{kor / MAX_SCORE:>12.1%}")\n# TODO: english, math 줄을 추가하세요\n# TODO: 총점과 평균(소수 두 자리)을 계산해 출력하세요\n',
            solution: 'MAX_SCORE = 100\n\nname = input("이름 : ")\nkor = int(input("국어 : "))\neng = int(input("영어 : "))\nmat = int(input("수학 : "))\n\ntotal = kor + eng + mat\naverage = total / 3\nline = "-" * 30\n\nprint("=" * 30)\nprint(f"{name} 님의 성적표")\nprint("=" * 30)\nprint(f"{\'subject\':<10}{\'score\':>8}{\'ratio\':>12}")\nprint(line)\nprint(f"{\'korean\':<10}{kor:>8}{kor / MAX_SCORE:>12.1%}")\nprint(f"{\'english\':<10}{eng:>8}{eng / MAX_SCORE:>12.1%}")\nprint(f"{\'math\':<10}{mat:>8}{mat / MAX_SCORE:>12.1%}")\nprint(line)\nprint(f"{\'total\':<10}{total:>8}")\nprint(f"{\'average\':<10}{average:>8.2f}{average / MAX_SCORE:>12.1%}")\nprint("=" * 30)\n',
            stdin: '홍길동\n92\n87\n78\n',
            expect: '이름 : 홍길동\n국어 : 92\n영어 : 87\n수학 : 78\n==============================\n홍길동 님의 성적표\n==============================\nsubject      score       ratio\n------------------------------\nkorean          92       92.0%\nenglish         87       87.0%\nmath            78       78.0%\n------------------------------\ntotal          257\naverage      85.67       85.7%\n==============================' }
        ],
        quiz: [
          ...
          { q: '다음 중 결과가 <b>True</b> 인 것은?', options: ['<code>0.1 + 0.2 == 0.3</code>', '<code>bool("0")</code>', '<code>None == 0</code>', '<code>type(True) == int</code>'], answer: 1,
            explain: '글자가 하나라도 들어 있는 문자열은 참입니다(<code>"0"</code> 도 참!). 실수는 오차 때문에 <code>==</code> 비교가 False 이고, None 은 0 과 다르며, <code>type(True)</code> 는 <code>bool</code> 입니다. (단 <code>isinstance(True, int)</code> 는 True 입니다)' }
        ],
        slides: [
...
print(a)`,
          ...
          { layout: 'code', title: '보충: input() 은 문자열 · 숫자가 아니면?', code: 'a = input("첫 번째 수 : ")\nb = input("두 번째 수 : ")\nprint(a + b)\nprint(int(a) + int(b))\n\ntry:\n    print(int("열"))\nexcept ValueError as e:\n    print("오류:", e)',
            stdin: '100\n200\n',
            points: ['input() 결과는 항상 str', '"100" + "200" → "100200"', 'int() · float() 로 바꿔서 계산', '숫자가 아니면 ValueError → try/except 로 안내'],
            notes: '<p>학생들이 가장 많이 겪는 실수입니다. [프로그램 2]에서 <code>int(input(…))</code> 을 쓴 이유와 연결합니다.</p><p>try/except 는 13장 내용이지만, <b>실제 프로그램은 잘못된 입력을 늘 만난다</b>는 감각을 여기서 심어 줍니다. “오류로 멈출까, 안내하고 계속할까?”</p><p>시간: 5분</p>' },
          { layout: 'code', title: '한 걸음 더: 실수의 오차와 안전한 비교', code: 'import math\nfrom decimal import Decimal\n\nprint(0.1 + 0.2)\nprint(0.1 + 0.2 == 0.3)\nprint(format(0.1, ".20f"))\nprint(math.isclose(0.1 + 0.2, 0.3))\nprint(Decimal("0.1") + Decimal("0.2"))\nprint(Decimal(0.1))',
            points: ['0.1 은 2진수로 정확히 못 쓴다', '실수끼리 == 비교는 위험', 'math.isclose() = “거의 같은가”', '돈 계산은 Decimal("0.1") — 문자열로!'],
            notes: '<p>마지막 줄이 압권입니다 — <code>Decimal(0.1)</code> 이 0.1000…0555 로 나오면서 “컴퓨터가 실제로 저장한 값”이 드러납니다.</p><p><b>발문</b>: “쇼핑몰 결제 금액을 float 으로 계산하면 어떤 일이 생길까요?”</p><p>시간: 5분</p>' },
          { layout: 'two', title: '한 걸음 더: 형 확인과 진리값',
            left: { title: 'type() vs isinstance()', code: 'print(type(10) == int)\nprint(isinstance(10, int))\nprint(isinstance(3.14, (int, float)))\nprint(type(True) == int)\nprint(isinstance(True, int))\nprint(True + True)' },
            right: { title: 'None 과 참 · 거짓', code: 'result = None\nprint(result is None)\nprint(bool(0), bool(""), bool(None))\nprint(bool("0"), bool(" "))' },
            notes: '<p>왼쪽: 실무에서는 <code>isinstance()</code> 를 씁니다(여러 형을 한 번에, 자식 형 포함). <code>True + True == 2</code> 에서 학생들이 놀랍니다 — bool 은 int 의 자식.</p><p>오른쪽: <code>bool("0")</code> 이 True 라는 점이 핵심 함정입니다. None 비교는 <code>==</code> 가 아니라 <code>is</code>.</p><p>시간: 5분</p>' },
          ...
          { layout: 'practice', title: '🚀 프로젝트: 성적표 서식 출력기', desc: '이름과 세 과목 점수를 입력받아 총점 · 평균(소수 2자리) · 달성률(%)을 표 모양으로 출력하세요. 만점은 MAX_SCORE 상수로.',
            starter: 'MAX_SCORE = 100\n\nname = input("이름 : ")\nkor = int(input("국어 : "))\neng = int(input("영어 : "))\nmat = int(input("수학 : "))\n\nline = "-" * 30\nprint("=" * 30)\nprint(f"{name} 님의 성적표")\nprint("=" * 30)\nprint(f"{\'korean\':<10}{kor:>8}{kor / MAX_SCORE:>12.1%}")\n# TODO: english · math · 총점 · 평균 줄을 추가하세요\n',
            solution: 'MAX_SCORE = 100\n\nname = input("이름 : ")\nkor = int(input("국어 : "))\neng = int(input("영어 : "))\nmat = int(input("수학 : "))\n\ntotal = kor + eng + mat\naverage = total / 3\nline = "-" * 30\n\nprint("=" * 30)\nprint(f"{name} 님의 성적표")\nprint("=" * 30)\nprint(f"{\'korean\':<10}{kor:>8}{kor / MAX_SCORE:>12.1%}")\nprint(f"{\'english\':<10}{eng:>8}{eng / MAX_SCORE:>12.1%}")\nprint(f"{\'math\':<10}{mat:>8}{mat / MAX_SCORE:>12.1%}")\nprint(line)\nprint(f"{\'total\':<10}{total:>8}")\nprint(f"{\'average\':<10}{average:>8.2f}{average / MAX_SCORE:>12.1%}")\nprint("=" * 30)\n',
            stdin: '홍길동\n92\n87\n78\n',
            notes: '<p>3장 전체를 한 번에 쓰는 마무리 과제입니다. 표가 어긋나면 서식 폭(10 · 8 · 12)을 다시 확인하게 하세요.</p><p>같은 줄이 세 번 반복되는 것을 학생이 답답해하면 성공입니다 — “그 해답이 7장 반복문과 8장 리스트”라고 예고합니다.</p><p>시간: 12분</p>' },
          { layout: 'practice', title: '실습 3-22. 초를 분과 초로', desc: '초를 입력받아 "2분 15초" 와 "2:15" 형식으로 출력하세요. (// 와 %, %02d 활용)',
            starter: 'sec = int(input("초 입력 : "))\n# TODO: 분과 남은 초를 계산해 출력\n',
            solution: 'sec = int(input("초 입력 : "))\nminute = sec // 60\nremain = sec % 60\nprint("%d분 %d초" % (minute, remain))\nprint("%d:%02d" % (minute, remain))\n',
            stdin: '135\n',
